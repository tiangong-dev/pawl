...
**固定阈值不是棘轮。** SonarQube 默认的门禁是拿固定条件去卡*新代码*，而"新代码"本身是一个可以配置的窗口——上一个版本以来、最近 N 天、或者和参考分支的 diff；自定义门禁还可以再加上针对整仓代码的条件。但阈值是有人划的一条线，比如覆盖率不得低于 80%。棘轮没有这条线：标准就是这个数上次被记下来时的值，所以每次数字变好，门槛自己就往上抬一格，也不需要谁先就"多少算够"达成一致。

**diff 过滤没有记忆。** `golangci-lint --new-from-rev` 和 `reviewdog -filter-mode=added` 默认把问题收窄到改动过的行，这很有用，也很便宜。但有些数字变差的时候，没有任何一行是被人改过的：依赖装进来一堆代码、产物体积涨了、某个测试开始被 skip。这些过滤器都看不见。
