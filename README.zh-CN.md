...
```yaml
- uses: tiangong-dev/pawl@v0.5.0   # 把 pawl 二进制放进 PATH——无需 Go/Node
  with:
    version: v0.5.0                # 可选;默认取最新 release
- run: pawl check
...
pawl 是单个二进制——任何 CI 里跑 `npx -y @pawl-tools/cli@0.5.0 check`(或下载 release 二进制)即可。
